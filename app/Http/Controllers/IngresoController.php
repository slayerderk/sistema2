...
use Carbon\Carbon;  //captura la hora y fecha de manera automatica
use App\Ingreso;
...
class IngresoController extends Controller
{
    ...
    public function pdf(Request $request,$id){
        ...
        //nombre de reporte + numero de documento
        $numingreso=Ingreso::select('num_comprobante')->where('id',$id)->get();
        $serieingreso=Ingreso::select('serie_comprobante')->where('id',$id)->get();
        //variable pdf
        $pdf = \PDF::loadView('pdf.ingreso',['ingreso'=>$ingreso,'detalles'=>$detalles]);
        return $pdf->download('ingreso_'.$serieingreso[0]->serie_comprobante.'-'.$numingreso[0]->num_comprobante.'.pdf');
    } 
    public function store(Request $request)
    {
        ...
        try{
            ...
            //esto sirve para las notificaciones 
            ...
            //avisa a todos los usuarios sobre la venta o ingreso hecho        
            $allUsers = User::all();
